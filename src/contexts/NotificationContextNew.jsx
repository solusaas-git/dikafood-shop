...
export const NotificationProvider = ({ children }) => {
  ...
  const error = useCallback((message, options = {}) => {
    ...
    if (typeof message === 'object') {
      displayMessage = message?.message || JSON.stringify(message) || 'Erreur inconnue';
    }
    return addNotification(displayMessage, {
      ...options,
  ...
  const apiError = useCallback((error, options = {}) => {
    let message = error?.response?.data?.message || error?.message || 'Une erreur est survenue lors de la communication avec le serveur.';
    if (typeof message === 'object') {
      message = message?.message || JSON.stringify(message) || 'Erreur inconnue';
    }
    return addNotification(message, {
      ...options,
      type: NOTIFICATION_TYPES.ERROR,
